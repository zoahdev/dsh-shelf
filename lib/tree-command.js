 ...
 *
 * Supported host: `dsh --profile pi-tui` only.
 * `dsh --profile web` / `dsh web` is out of scope — its question UI is not
 * a tree navigator, so `/nav` is not registered there.
 * Double-Esc is not available in pi-tui (Esc is interrupt / browse).
 * Filter is a first picker step (host Ctrl+O expands tool output).
 *
...
/** True when this process is the official web profile (`dsh web`). */
export function isDshWebProfile(argv = process.argv) {
  const profile = argv.indexOf('--profile')
  if (profile !== -1 && argv[profile + 1] === 'web') return true
  const bin = argv[1] ?? ''
  return /(?:^|\/)dsh$|(?:^|\/)bin\.js$/.test(bin) && argv[2] === 'web'
}
